...
(function(){
  ...
  function renderParagon(showGlyph=false){
    ...
    const specials = def.specials.map(s=>{ const p=rotPoint(s.x,s.y,b.rotation); const isGlyph=s.type==='glyph'; const text=isGlyph && b.glyph ? b.glyph.slice(0,3) : s.label; return `<button class="imgSpecial ${s.type}" style="left:${p.x}%;top:${p.y}%" onclick="${isGlyph?'openGlyphPicker()':'toggleSpecialNode(\\''+s.id+'\\')'}" title="${isGlyph?'Glyph Socket':s.type}">${text}</button>`; }).join('');
